                ...
                <table id="datatable" class="table table-striped table-bordered">
                        ...
                        @foreach($purchases as $key => $purchase)
                            <tr>
                                ...
                                <td> {{number_format($purchase->total_paid_amount, 2) }}</td>
                                <td> 
                                 {{ number_format(($purchase->total_amount-$purchase->total_paid_amount),2) }} 
                                </td>
                                <td>  
                                 {{ number_format(($purchase->total_amount - $purchase->total_paid_amount),2) }} 
                                </td>
                                <td> 
                                <button value="{{ $purchase->invoice_number }}" class="btn-rounded btn-sm btn-info waves-effect waves-light addPayment"> <i class="fa fa-plus"></i> Pay </button>
                                <a href="{{ url('suppliers/payments/'.$purchase->invoice_number) }}" class="btn-rounded btn-sm btn-success waves-effect waves-light addPayment"> <i class="fa fa-eye"></i> View </a>
                                </td>       
                            </tr>
                          @endforeach  
                            </tbody>
...
<form id="AddPaymentForm" method="post" action="{{ url('suppliers/purchase-payments/add-payment') }}">
             @csrf
        <div class="modal-dialog modal-md">
            <div class="modal-content">
                <div class="modal-header">
                    <button type="button" class="close" data-dismiss="modal" aria-hidden="true">×</button>
                    <h4 class="modal-title" id="mySmallModalLabel">Add Supplier Payment  </h4>
                </div>
                ...
                <div class="row">
                    ...
                    <div class="row">
                        <div class="col-md-6">
                            <div class="form-group">
                                <label for="inputEmail3" class="control-label"> Invoice Amount (VAT inclusive) : </label>
                    ...
                    <div class="row">
                        <div class="col-md-6">
                            <div class="form-group">
                                ...
                                <input type="text" name="paid_amount" id="paid_amount" class="form-control" required placeholder="Enter Amount to Pay">
                             </div>
                        </div>
                        <div class="col-md-6">
                             <div class="form-group">
                                <label for="" class="control-label">Date </label>
                                <input type="text" class="form-control" autocomplete="off" name="created_date" placeholder="Date Paid" id="datepicker-autoclose" data-date-format="yyyy-mm-dd" required>
                            </div>
                        </div>
                    </div>
                    ...
                    <div class="modal-footer">
                        ...
                        <button type="submit" class="btn-rounded btn-primary waves-effect waves-light">Save Payment </button>
                    </div>
                </div>
            </div><!-- /.modal-content -->
        </div><!-- /.modal-dialog -->
    ...
    $('.addPayment').click(function() {
         ...
         $.ajax({
             ...
             success: function (response) {
                 ...
                 var a = parseFloat(response[0].total,2);
                 var b = parseFloat(response[0].calculated_vat_amount,2);
